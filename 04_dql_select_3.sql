...
from
    tbl_menu
group by
    category_code; #catagory_code 컬럼 값이 같은 행을 그룹화
...
# 카테고리 테이블에서
# 부모 카테고리(ref_category_code) 별로 개수 3개 이상
# 부모 카테고리 번호, 개수 조회
# 부모 카테고리 오름 차순으로 조회
# null값은 제외
select
...
group by
    ref_category_code
having
    # ref_category_code is not null #where절에서 필터링이 효율적
    # and
    count(*) >= 3
order by
